...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="card bg-base-100 shadow-xl border border-base-200/50 overflow-hidden">
        ...
        <div class="h-1.5 bg-gradient-to-r from-info via-primary to-secondary"></div>

        <div class="card-body p-8">
            <h2 class="card-title text-2xl font-extrabold tracking-tight mb-1">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-info" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                Edit Resource
            </h2>
            <p class="text-base-content/60 mb-6">Update the details of <strong>{{ $resource->title }}</strong>.</p>
            ...
            <form action="{{ route('resources.update', $resource) }}" method="POST" class="space-y-5">
                ...
                <div class="form-control w-full">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $resource->title) }}" placeholder="e.g. Laravel Documentation" class="input input-bordered w-full @error('title') input-error @enderror" />
                    @error('title')
                ...
                <div class="form-control w-full">
                    ...
                    <input type="url" id="url" name="url" value="{{ old('url', $resource->url) }}" placeholder="https://laravel.com/docs" class="input input-bordered w-full @error('url') input-error @enderror" />
                    @error('url')
                        <label class="label"><span class="label-text-alt text-error">{{ $message }}</span></label>
                    @enderror
                </div>
                ...
                <div class="form-control w-full">
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="A short description of this resource..." class="textarea textarea-bordered w-full @error('description') textarea-error @enderror">{{ old('description', $resource->description) }}</textarea>
                    @error('description')
                        <label class="label"><span class="label-text-alt text-error">{{ $message }}</span></label>
                    @enderror
                </div>
                ...
                <div class="form-control w-full">
                    ...
                    <input type="text" id="tags" name="tags" value="{{ old('tags', is_array($resource->tags) ? implode(', ', $resource->tags) : $resource->tags) }}" placeholder="laravel, php, backend" class="input input-bordered w-full @error('tags') input-error @enderror" />
                    @error('tags')
                        <label class="label"><span class="label-text-alt text-error">{{ $message }}</span></label>
                    @enderror
                </div>
                ...
                <div class="form-control">
                    <label class="label cursor-pointer justify-start gap-3" for="is_favorite">
                        <input type="checkbox" id="is_favorite" name="is_favorite" value="1" {{ old('is_favorite', $resource->is_favorite) ? 'checked' : '' }} class="checkbox checkbox-primary" />
